...
export class RoomRenderer {
  ...
  private furnitureSpriteKeys: Set<string> = new Set();
  ...
  renderFurniture(items: FurnitureItem[]): void {
    if (!this.camera || !this.mounted) return;
    void this.syncFurniture(items);
  }

  private furnitureKey(id: string): string {
    return `furniture:${id}`;
  }

  private async syncFurniture(items: FurnitureItem[]): Promise<void> {
    if (!this.camera || !this.mounted) return;

    const nextIds = new Set(items.filter((i) => i.visibility).map((i) => i.id));

    for (const key of [...this.furnitureSpriteKeys]) {
      const id = key.replace(/^furniture:/, "");
      if (!nextIds.has(id)) {
        const wrapper = this.objectSprites.get(key);
        wrapper?.parent?.removeChild(wrapper);
        wrapper?.destroy({ children: true });
        this.objectSprites.delete(key);
        this.furnitureSpriteKeys.delete(key);
      }
    }

    for (const item of items) {
      if (!item.visibility) continue;
      const key = this.furnitureKey(item.id);
      ...
      this.objectSprites.set(key, wrapper);
      this.furnitureSpriteKeys.add(key);

      if (isBackendImageUrl(item.sprite)) {
        try {
          const texture = await Assets.load<Texture>(item.sprite);
          if (!this.mounted || !this.objectSprites.has(key)) return;
          texture.source.scaleMode = "nearest";
          const sprite = new Sprite(texture);
          sprite.anchor.set(0.5, 1);
          const targetHeight =
            (item.metadata?.height as number | undefined) ??
            FURNITURE_SPRITE_HEIGHT;
          const aspect = texture.width / texture.height || 1;
          sprite.height = targetHeight;
          sprite.width = targetHeight * aspect;
          wrapper.removeChildren();
          wrapper.addChild(sprite);
        } catch (err) {
          console.warn("[RoomRenderer] Failed to load furniture image:", item.sprite, err);
          wrapper.addChild(createPlaceholderForObject(item));
        }
      } else {
        wrapper.addChild(createPlaceholderForObject(item));
      }
  ...
  private clearObjectSprites(): void {
    ...
    this.furnitureSpriteKeys.clear();
